...
export default function SocialProfiles() {
  ...
  return <nav className="social-profiles" aria-label={t('Social profiles')}>
    {socialLinks.map(link => <a key={link.id} href={link.href} target="_blank" rel="noopener noreferrer" aria-label={`${link.label} (${t('opens in a new tab')})`}><SocialIcon id={link.id} /><span>{link.label}</span><span aria-hidden="true">↗</span></a>)}
  </nav>;
}
